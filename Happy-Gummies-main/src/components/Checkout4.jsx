...
// function Checkout4() {
//   const { state } = useLocation(); // address, orderId, etc.

//   return (
//     <div className="checkout">
//       <div className="checkout-container">
//         {/* <OrderConfirmation orderId={state?.orderId} address={state?.address} /> */}
//         <OrderConfirmation orderId={location.state?.orderId} />
...
function Checkout4() {
  ...
  const state = location.state; // contains orderId

   useEffect(() => {
    if (!state || !state.orderId) {
      // Redirect to homepage if no state found
      navigate("/");
    }
  }, [state]);
  ...
  return (
    <div className="checkout">
      <div className="checkout-container">
        <OrderConfirmation orderId={state?.orderId} />
      </div>
    </div>
  );
}
